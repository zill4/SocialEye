...
def read_all():
    """
    This function responds to a request for /api/g_comp
...
def read_one(bname):
    """
    This function responds to a request for /api/g_comp/{bname}
...
def update(bname, brand):
    ...
    if collection.find_one({"bname": bname}):
        collection.update_one({"bname": bname}, {"bname": g_comp.get("bname")})
